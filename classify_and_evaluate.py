...
def classify(texts_to_classify):
    ...
    with torch.no_grad():
        outputs = model.generate(inputs['input_ids'], attention_mask=inputs['attention_mask'], max_length=150, num_beams=2, early_stopping=True)
...
def evaluate():
    ...
    num_batches = len(dataset['test']) // batch_size + (0 if len(dataset['test']) % batch_size == 0 else 1)
